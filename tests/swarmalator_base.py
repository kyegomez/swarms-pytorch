...
def test_function_for_x_shape():
    ...
    dx = function_for_x(xi, sigma_i, N, J, alpha, beta, gamma, epsilon_a, epsilon_r, R, D)
    assert dx.shape == (N, D)
...
def test_function_for_sigma_shape():
    xi = torch.randn(N, D)
    sigma_i = torch.randn(N, D)
    d_sigma = function_for_sigma(xi, sigma_i, N, J, alpha, beta, gamma, epsilon_a, epsilon_r, R, D)
...
def test_simulate_swarmalators_non_zero_initial_condition():
    ...
    sigma_i = torch.randn(N, D)
    results_xi, results_sigma_i = simulate_swarmalators(
        N, J, alpha, beta, gamma, epsilon_a, epsilon_r, R, D, T=T, dt=dt, xi=xi, sigma_i=sigma_i
    )
